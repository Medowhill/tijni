...
object Interpreter {
  ...
  def run(pg: Program): Unit = {
    var ctx = Ctx(
      Frame(Vector(), Nil, 0, pg.mainMethod) :: Nil,
      Map(),
      pg
    )
    // println(ctx)
  ...
  def step(ctx: Ctx): Ctx = {
    val nctx = ctx.instruction match {
      ...
      case Getstatic(_) => ctx.getPrintln
  ...
  case object Println extends Value
  ...
  case class Frame(
    ...
    def store(i: Int): Frame = {
      val newVars =
        if (variables.isDefinedAt(i)) variables.updated(i, head)
        else (0 to i).toVector.map(
          j =>
            variables.applyOrElse(j, (j: Int) => if (i == j) head else Undef)
        )
      copy(variables = newVars, stack = tail)
    }
  ...
  case class Ctx(
    ...
    def getPrintln: Ctx =
      copy(callStack = head.push(Println) :: tail)
